...
function init() {
    try {
        config = JSON.parse(fs.readFileSync(__dirname + '/config1.json', 'utf8'));
    } catch (err) {
        console.log("config1.json is missing or invalid file, Rerun the program with right file")
        process.exit();
    }
...
function query2() {
    var args = getArgs(config.queryRequest2);
    // Construct the query request
    var queryRequest = {
        // Name (hash) required for query
        chaincodeID: chaincodeID,
        // Function to trigger
        fcn: config.queryRequest2.functionName,
        // Existing state variable to retrieve
        args: args
    };
